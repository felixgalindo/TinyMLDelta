/*
 * demo_app_ei.cpp — TinyMLDelta + Edge Impulse SDK Demo (UNO Q Linux)
 *
 * Demonstrates TinyMLDelta integration with the Edge Impulse SDK.
 * Uses EI's external model loading hook (EI_CLASSIFIER_EXTERNAL_MODEL_LOADING)
 * so run_classifier() reads models from TinyMLDelta-managed storage.
 * TinyMLDelta handles model patching; EI SDK handles DSP + inference.
 *
 * Prerequisites:
 *   1. Export your Edge Impulse project as a C++ library
 *   2. Build with: make EI_SDK=/path/to/edge-impulse-sdk-pack
 *   3. The EI SDK must include the external model loading patch
 *      (feature/external-model-loading branch)
 *
 * Compiled and deployed to the Arduino UNO Q Qualcomm Linux co-processor.
 * Connects to arduino-router's Unix socket (MsgPack-RPC).
 *
 * Build:   make EI_SDK=/path/to/edge-impulse-sdk-pack
...
/* TinyMLDelta C library + in-memory flash port (shared with tflite/) */
...
static constexpr int         kWindowSize  = EI_CLASSIFIER_RAW_SAMPLE_COUNT;
...
void log_msg(const char *level, const char *fmt, ...) {
    ...
    struct tm t{};
    localtime_r(&now, &t);
    fprintf(stderr, "%02d:%02d:%02d [demo_app_ei] %s ",
...
/* MsgPack codec and RouterClient (shared with tflite/). */
#include "msgpack.h"
...
// =============================================================================
// Edge Impulse External Model Loader — TinyMLDelta bridge
//
// This callback is registered with the EI SDK's ei_config_tflite_graph_t.
// When run_classifier() needs the model, it calls this instead of reading
// from a compiled-in const array.
//
// The loader supports two modes:
//   1. Zero-copy (XIP flash): return a direct pointer via *model_out.
//      No RAM buffer needed — the model is read directly from flash.
//      Example for a bare-metal MCU:
//        const tmd_slot_t *s = &tmd_layout()->slotA;
//        *model_out = (const unsigned char *)s->addr;  // XIP pointer
//        *model_size_out = s->size;
//
//   2. Copy mode: if *model_out is left NULL, the SDK allocates a RAM
//      buffer and calls again with buf != NULL. The loader copies the
//      model into buf.
//
// This Linux demo uses copy mode (model is a file on disk, not XIP).
// =============================================================================

static bool tmd_ei_model_loader(const unsigned char **model_out,
                                size_t *model_size_out,
                                unsigned char *buf, size_t buf_size) {
    auto data = read_file(kModelPath);
    if (data.empty()) {
        LOG_ERROR("EI model loader: cannot read %s", kModelPath);
        return false;
    }

    *model_size_out = data.size();

    if (!buf) {
        // First call: report size, request copy mode (no XIP on Linux)
        if (model_out) *model_out = NULL;
        return true;
    }

    // Second call: copy into provided buffer
    if (data.size() > buf_size) {
        LOG_ERROR("EI model loader: model too large (%zu > %zu)", data.size(), buf_size);
        return false;
    }
    memcpy(buf, data.data(), data.size());
    LOG_INFO("EI model loader: loaded %zu bytes from %s", data.size(), kModelPath);
    return true;
}

// =============================================================================
// ModelRunner — Edge Impulse inference + TinyMLDelta patch apply
// =============================================================================

class ModelRunner {
public:
    explicit ModelRunner(const char *model_path) : model_path_(model_path) {
        // Check if model file exists
        struct stat st;
        if (::stat(model_path, &st) == 0) {
            model_available_ = true;
            LOG_INFO("Model available for EI SDK: %s (%ld bytes)",
                     model_path, (long)st.st_size);
        } else {
            LOG_WARN("Model not found: %s", model_path);
        }
    }
    ...
    /*
     * Run inference using Edge Impulse's run_classifier().
     *
     * The EI SDK internally:
     *   1. Calls tmd_ei_model_loader() to get model bytes from flash/file
     *   2. Sets up the TFLite interpreter (re-parses if model changed)
     *   3. Runs DSP preprocessing on raw features
     *   4. Invokes the TFLite model
     *   5. Returns classification/anomaly results
     *
     * Returns the anomaly score, or -1.0f if inference fails.
     */
    float infer(const float *features, size_t features_len) {
        std::lock_guard<std::mutex> lk(lock_);
        if (!model_available_) return -1.0f;

        // Wrap raw features in an EI signal
        signal_t signal;
        signal.total_length = features_len;
        signal.get_data = [](size_t offset, size_t length, float *out_ptr) -> int {
            // This lambda captures nothing — features are passed via the
            // static thread-local below to avoid lambda capture issues.
            memcpy(out_ptr, s_features_ + offset, length * sizeof(float));
            return 0;
        };

        // Store features pointer for the lambda (single-threaded inference)
        s_features_ = features;

        ei_impulse_result_t result = {};
        EI_IMPULSE_ERROR err = run_classifier(&signal, &result, /*debug=*/false);

        if (err != EI_IMPULSE_OK) {
            LOG_ERROR("EI run_classifier failed: %d", (int)err);
            return -1.0f;
        }

        // For anomaly detection models, EI returns result.anomaly
        // For classification models, you'd read result.classification[]
        float score = result.anomaly;
        LOG_INFO("EI inference: anomaly_score=%.4f (timing: dsp=%d ms, classify=%d ms)",
                 score,
    ...
    /*
     * Apply a TinyMLDelta .tmd patch and update the model file.
     * After patching, the next run_classifier() call picks up the new model
     * via tmd_ei_model_loader() automatically.
     */
    bool apply_patch(const char *patch_path) {
        ...
        // Write updated model to file — tmd_ei_model_loader() reads from here
        ...
        // Mark model available — next run_classifier() call will load it
        // via tmd_ei_model_loader() and the EI SDK will re-parse automatically
        model_available_ = true;
        return true;
    }
...
private:
    ...
    // Thread-local features pointer for the EI signal lambda
    static thread_local const float *s_features_;
};

thread_local const float *ModelRunner::s_features_ = nullptr;

// =============================================================================
// Normalization config — loaded from model_config.json
// =============================================================================
...
private:
    ...
    std::string toggle_inference() {
        ...
        state_.readings = 0;
        memset(state_.window, 0, sizeof(state_.window));
        std::string msg = "[INFER] Started. Engine: Edge Impulse SDK + TinyMLDelta.";
        if (!norm_.loaded)
    ...
    std::string inference_tick(float temp_c) {
        ...
        float score = model_.infer(window_snap, kWindowSize);
        if (score < 0.0f)
            return "[INFER] No model loaded. Push a model and send 'u' to apply.";

        bool anom = score > kAnomalyMse;
        {
            std::lock_guard<std::mutex> lk(lock_);
            state_.last_score = score;
            state_.anomaly    = anom;
        }

        char line[128];
        snprintf(line, sizeof(line), "TEMP %.2f C  score=%.4f%s",
                 temp_c, score, anom ? "  *** ANOMALY ***" : "");
        LOG_INFO("%s", line);
        return line;
    }
    ...
    std::string do_update() {
        ...
        bool ok = model_.apply_patch(kPatchPath);
        ::remove(kPatchPath);
    ...
    std::string do_status() {
        ...
        snprintf(buf, sizeof(buf),
            "--- STATUS ----------------------------------------\n"
            "Mode         : %s\n"
            "Engine       : Edge Impulse SDK + TinyMLDelta\n"
            "Model        : %s\n"
...
int main() {
    LOG_INFO("TinyMLDelta + Edge Impulse demo starting");
    LOG_INFO("Engine : Edge Impulse SDK (external model loading)");
